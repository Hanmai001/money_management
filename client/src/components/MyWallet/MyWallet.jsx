...
function FormCurrency(props) {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const res = await axios.get("/api/currency.json");
      ...
      <div className={clsx(styles.containerCurrency)}>
        ...
        <div className={styles.lstCurrency}>
          {lstCurrency.map(curr => (
            <div key={curr._id} className={clsx("col-12", styles.iCurrency)} onClick={() => props.chooseCurrency(curr)}>
...
function Wallet(props) {
  ...
  return (
    <div className={clsx("col-12", styles.iWallet)}>
      ...
      <button className={styles.edit} onClick={() => props.openFormEdit({ _id: props.id, name: props.name, currency: props.currency, userId: props.userId, initialBalance: props.initialBalance })}>Edit</button>
